...
from config import *
from utils import setup_academic_style, setup_project_logging
...
def main():
    ...
    for group in EXPERIMENT_GROUPS:
        ...
        if not os.path.exists(csv_path): 
            logging.warning(f"跳过实验组 [{group['label']}]: 缺少提取的 CSV 指标文件。")
            continue
        ...
        try:
            ...
            logging.error(f"加载组别数据失败 [{group['label']}]: {e}")
    ...
    logging.info(f"所有盐底辟演化对比图谱已导出至: {FINAL_OUTPUT_DIR}")
